import React from 'react';
import { Pencil, Play, Settings, Loader2 } from 'lucide-react';
import ProjectDropdown from './ProjectDropdown';
import { Project } from '../services/projectService';

interface TopBarProps {
  isDeploying?: boolean;
  onToggleWorkflowSelector?: () => void;
  showWorkflowSelector?: boolean;
  onTestWorkflow?: () => void;
  onEditMode?: () => void;
  isTestMode?: boolean;
  onSettingsClick?: () => void;
  currentProject?: Project | null;
  onProjectChanged?: (project: Project) => void;
}

const TopBar: React.FC<TopBarProps> = ({ 
  isDeploying = false,
  onToggleWorkflowSelector,
  showWorkflowSelector = false,
  onTestWorkflow,
  onEditMode,
  isTestMode = false,
  onSettingsClick,
  currentProject = null,
  onProjectChanged,
}) => {
  return (
    <div style={{
      height: '48px',
      backgroundColor: 'transparent',
      display: 'flex',
      alignItems: 'center',
      justifyContent: 'space-between',
      padding: '0 16px',
    }}>
      {/* Left Section */}
      <div style={{
        display: 'flex',
        alignItems: 'center',
        gap: '12px',
      }}>
        <span style={{
          fontSize: '13px',
          fontWeight: 500,
          color: '#ffffff',
        }}>
          FlowDelsuc
        </span>
        {onProjectChanged && (
          <ProjectDropdown
            currentProject={currentProject}
            onProjectChanged={onProjectChanged}
          />
        )}
      </div>

      {/* Center Section - Segmented Button */}
      <div style={{
        display: 'flex',
        alignItems: 'center',
      }}>
        <div style={{
          display: 'flex',
          backgroundColor: '#2a2a2a',
          borderRadius: '8px',
          overflow: 'hidden',
          border: '1px solid #2a2a2a',
        }}>
          <button 
            onClick={isTestMode && onEditMode ? onEditMode : undefined}
            style={{
              padding: '6px 12px',
              border: 'none',
              backgroundColor: !isTestMode ? '#3b82f6' : '#2a2a2a',
              color: !isTestMode ? '#ffffff' : '#9ca3af',
              cursor: 'pointer',
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'center',
              borderRight: '1px solid #1a1a1a',
              transition: 'background-color 0.2s',
            }}
            onMouseEnter={(e) => {
              if (!isTestMode) {
                e.currentTarget.style.backgroundColor = '#2563eb';
              } else {
                e.currentTarget.style.backgroundColor = '#3a3a3a';
              }
            }}
            onMouseLeave={(e) => {
              if (!isTestMode) {
                e.currentTarget.style.backgroundColor = '#3b82f6';
              } else {
                e.currentTarget.style.backgroundColor = '#2a2a2a';
              }
            }}
          >
            <Pencil size={16} />
          </button>
          <button 
            onClick={onTestWorkflow}
            disabled={isDeploying}
            style={{
              padding: '6px 12px',
              border: 'none',
              backgroundColor: isTestMode ? '#3b82f6' : isDeploying ? '#3a3a3a' : '#2a2a2a',
              color: isTestMode ? '#ffffff' : isDeploying ? '#6b7280' : '#9ca3af',
              cursor: isDeploying ? 'not-allowed' : 'pointer',
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'center',
              transition: 'background-color 0.2s',
              opacity: isDeploying ? 0.6 : 1,
            }}
            onMouseEnter={(e) => {
              if (!isDeploying) {
                if (isTestMode) {
                  e.currentTarget.style.backgroundColor = '#2563eb';
                } else {
                  e.currentTarget.style.backgroundColor = '#3a3a3a';
                }
              }
            }}
            onMouseLeave={(e) => {
              if (!isDeploying) {
                if (isTestMode) {
                  e.currentTarget.style.backgroundColor = '#3b82f6';
                } else {
                  e.currentTarget.style.backgroundColor = '#2a2a2a';
                }
              }
            }}
          >
            {isDeploying ? <Loader2 size={16} style={{ animation: 'spin 1s linear infinite' }} /> : <Play size={16} />}
          </button>
        </div>
      </div>

      {/* Right Section */}
      <div style={{
        display: 'flex',
        alignItems: 'center',
        gap: '8px',
      }}>
        <button 
          onClick={onSettingsClick}
          style={{
            padding: '6px',
            border: 'none',
            backgroundColor: 'transparent',
            color: '#9ca3af',
            cursor: 'pointer',
            display: 'flex',
            alignItems: 'center',
            justifyContent: 'center',
            borderRadius: '4px',
            transition: 'background-color 0.2s',
          }}
          onMouseEnter={(e) => e.currentTarget.style.backgroundColor = '#2a2a2a'}
          onMouseLeave={(e) => e.currentTarget.style.backgroundColor = 'transparent'}
        >
          <Settings size={18} />
        </button>
      </div>
    </div>
  );
};

export default TopBar;

